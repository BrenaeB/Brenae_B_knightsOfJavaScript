...
//This function will exceute Player Two's attack on Player One
function attackPlayerOne() {
    //This checks when it is Player Two's tunr to attack
    if (gameState.whoseTurn === 2) {
        //Defines the array of the image paths for Player Two's animations when idled or attacking
        let playerTwoFrames = ["./images/L_Idle.png", "./images/L_Attack.png"];
        // Get the HTML element for Player Two's sprite
        let playerSprite = document.getElementById("playerTwoSprite");
        //This  sets the sprite to the attack image
        playerSprite.src = playerTwoFrames[1];
        //Changes the sprite animation from 'idle' to 'attack'
        playerSprite.classList.remove("idle");
        playerSprite.classList.add("attack");
        let enemySprite = document.getElementById("playerOneSprite");
        enemySprite.classList.remove("idle");
        enemySprite.classList.add("damage");
        document.getElementById("SFX_PlayerDamage").play();
        //Reset the animations after a short delay
        setTimeout(() => {
            //Removes the damage class from Player One's sprite and changes to idle
            enemySprite.classList.remove("damage");
            enemySprite.classList.add("idle");
            //Changes Player Two's sprite back to the idle image
            playerSprite.src = playerTwoFrames[0];
            playerSprite.classList.remove("attack");// Removes the attack class
            playerSprite.classList.add("idle"); //Adds idle class
        }, 350);
        //Calls the function to switch turns Player one after the attack
        changePlayer();
    }
}
function changeButtonStatus() {
...
document.getElementById("playerOneAttack").addEventListener("click", attackPlayerTwo);
document.getElementById("playerTwoAttack").addEventListener("click", attackPlayerOne);
